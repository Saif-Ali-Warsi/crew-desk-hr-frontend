...
function ApplyLeavePage() {
  ...
  const validate = () => {
    ...
    if (!formData.employeeId) {
      newErrors.employeeId = t("applyLeave.employeeRequired");
    }

    if (!formData.leaveType) {
      newErrors.leaveType = t("applyLeave.leaveTypeRequired");
    }

    if (!formData.startDate) {
      newErrors.startDate = t("applyLeave.startDateRequired");
    }

    if (!formData.endDate) {
      newErrors.endDate = t("applyLeave.endDateRequired");
    }
    ...
    if (
      ...
      newErrors.endDate = t("applyLeave.endDateBeforeStart");
    }

    const trimmedReason = formData.reason.trim();

    if (!trimmedReason) {
      newErrors.reason = t("applyLeave.reasonRequired");
    } else if (trimmedReason.length < 5) {
      newErrors.reason = t("applyLeave.reasonMinLength");
    } else if (trimmedReason.length > 500) {
      newErrors.reason = t("applyLeave.reasonMaxLength");
    }
      ...
      >
        <div className="space-y-6">
          <div>
            ...
            <div className="relative">
              ...
              >
                <option value="">
                  {employeesLoading
                    ? t("applyLeave.loadingEmployees")
                    : t("applyLeave.selectEmployee")}
                </option>
          ...
          <div>
            ...
            <div className="relative">
              ...
              >
                <option value="">{t("applyLeave.selectLeaveType")}</option>

                <option value="CASUAL">{t("applyLeave.casual")}</option>

                <option value="SICK">{t("applyLeave.sick")}</option>

                <option value="EARNED">{t("applyLeave.earned")}</option>

                <option value="UNPAID">{t("applyLeave.unpaid")}</option>
              </select>
              <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3 text-slate-400">
                <svg className="h-4 w-4 fill-current" viewBox="0 0 20 20">
                  <path
          ...
          <div>
            ...
            <textarea
              ...
              disabled={loading}
              placeholder={t("applyLeave.reasonPlaceholder")}
              onChange={(event) =>
                setFormData({
                  ...formData,
        ...
        <div className="mt-8 flex items-center justify-end gap-3 border-t border-slate-100 pt-6">
          ...
          <button
            ...
            className="cursor-pointer inline-flex items-center justify-center rounded-lg bg-teal-700 px-4 py-2.5 text-sm font-semibold text-white shadow-xs transition-colors hover:bg-teal-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-teal-600 disabled:cursor-not-allowed disabled:bg-teal-400"
          >
            {loading ? (
              <>
                <svg
                ...
                {t("applyLeave.applying")}
              </>
            ) : (
              t("applyLeave.applyLeave")
            )}
          </button>
        </div>
      </form>
