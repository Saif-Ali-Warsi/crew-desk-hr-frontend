    ...
    const timer = setTimeout(() => {
      ...
      const fetchEmployees = async () => {
        try {
          ...
          setError(t("employee.failedToLoad"));
        } finally {
          setLoading(false);
        }
      };
  ...
  if (error) {
    return (
      <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">
        <p className="font-semibold">{t("employee.failedToLoad")}</p>
            ...
            <table className="w-full text-left border-collapse text-sm">
              <thead className="bg-gray-50 border-b border-gray-200 text-xs uppercase font-semibold text-gray-500">
                <tr className="text-white bg-teal-600">
                  <th className="px-6 py-3.5">{t("employee.name")}</th>
                  <th className="px-6 py-3.5">{t("employee.code")}</th>
                  <th className="px-6 py-3.5">{t("employee.email")}</th>
                  <th className="px-6 py-3.5">{t("employee.designation")}</th>
